...
@section('title', config('app.name') . ' - ' . __('level::attributes.levels_list'))
    ...
    <div id="create-toolbar-area">
        ...
        <div class="app-container container-fluid mb-7">
            <div id="create-collapse" class="collapse">
                @include('level::levels.partials.create')
            </div>
        </div>
    </div>
@endsection
            ...
            <div class="card-title">
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <input type="text" id="table-search" class="form-control form-control-solid w-250px ps-13"
                        placeholder="{{ __('level::placeholders.search_levels') }}" />
                </div>
            </div>
        </div>
            ...
            <table id="kt_datatable" class="table align-middle table-row-dashed fs-6 gy-5">
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th class="min-w-125px">{{ __('level::attributes.title_en') }}</th>
                        <th class="min-w-125px">{{ __('level::attributes.title_ar') }}</th>
                        <th class="min-w-125px">{{ __('level::attributes.publisher_id') }}</th>
                        <th class="min-w-125px">{{ __('level::attributes.category_id') }}</th>
                        @if ($is_super_admin)
                            <th class="min-w-125px">{{ __('publisher::attributes.tenant') }}</th>
                        @endif
                        <th class="min-w-125px">{{ __('level::attributes.is_active') }}</th>
                        <th class="min-w-125px">{{ __('level::attributes.created_at') }}</th>
                        <th class="text-end min-w-100px">{{ __('level::attributes.actions') }}</th>
                    </tr>
                </thead>
